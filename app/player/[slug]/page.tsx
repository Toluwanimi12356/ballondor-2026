...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!player) return {};
  const title = `${player.name} | Ballon d'Or 2026 Community Ballot`;
  const description = `Review ${player.name}'s verified candidate profile and cast your independent community ballot.`;
  const url = `/player/${player.slug}`;
  return {
    title,
    description,
...
export default async function PlayerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="profile-page">
    ...
    <PlayerVideo playerName={player.name} src={`/videos/players/${player.id}.mp4`} poster={`/images/video-posters/${player.id}.jpg?v=2`} />
    ...
    {player.competitions.length > 0 && <section className="profile-section">
